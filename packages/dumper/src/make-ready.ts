...
export async function fetchJson<T extends object>(url: string): Promise<T | null> {
   ...
   return response.json().catch(_ => null);
}

export async function unzip(stream: ReadableStream, basePath: string): Promise<void> {
   ...
   await stream.pipeTo(
      new UnzipStreamConsumer({
         async onFile(report, readable): Promise<void> {
...
export async function getSource(): Promise<ReadableStream> {
   ...
   Readable.fromWeb(writeToFile as any)
