...
export function readCapsuleFrameHeader(view: DataView, offset: number): { offset: number } & FrameDescriptor {
   const result: FrameDescriptor & { offset: number } = {
      body: null as unknown as FrameDescriptor['body'],
      offset,
   };
   ...
   if (isFragmented) {
      const fragment = (result.fragment = {} as Required<typeof result>['fragment']);
