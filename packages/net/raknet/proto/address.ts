...
// Constants for address binary sizes
export const IPV4_ADDRESS_BINARY_SIZE = 7; // IPv4 address size in bytes (1 byte for version + 4 bytes for address + 2 bytes for port)
export const IPV6_ADDRESS_BINARY_SIZE = 29; // IPv6 address size in bytes (1 byte for version  + 16 bytes for address + 2 bytes for port + 2 bytes for additional data)

export function writeAddress(dataview: DataView, offset: number, endpoint: ConnectionEndpoint): number {
   const isIpv4 = endpoint.address.indexOf('.') !== -1;
   dataview.setUint8(offset++, isIpv4 ? 4 : 6);
   if (isIpv4) {
      dataview.setUint32(offset, ipv4ToNumber(endpoint.address), false);

      // write port
      dataview.setUint16((offset += 4), endpoint.port, false);
...
export function readAddress(view: DataView, offset: number): ConnectionEndpoint & { version: number } {
   const family = view.getUint8(offset++);
   const isIpv4 = family === 4;
   const address: ConnectionEndpoint & { version: number } = { version: family, address: '', port: 0 };
   ...
   if (isIpv4) {
      ...
      let ipv6Address = '';
      for (let i = 0; i < 8; i++) {
         const part = view.getUint16(offset, false).toString(16).padStart(4, '0'); // Pad with zeros
         ipv6Address += part;
         if (i < 7) ipv6Address += ':'; // Add colon between parts
         offset += 2;
      }
...
export function parseIpv6AddressString(text: string): number[] {
   ...
   for (let i = 0, b = 0; b < address_part.length; ) {
      const part = address_part[b++];
      // If there is zero numbers cut
      if (part === '') {
         for (let n = 0, remains = 8 - i - address_part.length + b; n < remains; n++) parts[i++] = 0;
         continue;
      }
      //else way
